...
function studentiMovimentiEnsureTables(): void
{
    ...
    if (!studentiMovimentiColumnExists('studenti_movimenti_allegati', 'storage_type')) {
        dbExec("ALTER TABLE studenti_movimenti_allegati ADD COLUMN `storage_type` ENUM('LOCAL','DRIVE') NOT NULL DEFAULT 'LOCAL' AFTER `dimensione`");
    }
    if (!studentiMovimentiColumnExists('studenti_movimenti_allegati', 'drive_file_id')) {
        dbExec("ALTER TABLE studenti_movimenti_allegati ADD COLUMN `drive_file_id` VARCHAR(255) NULL AFTER `storage_type`");
    }
    if (!studentiMovimentiColumnExists('studenti_movimenti_allegati', 'drive_web_view_link')) {
        dbExec("ALTER TABLE studenti_movimenti_allegati ADD COLUMN `drive_web_view_link` VARCHAR(500) NULL AFTER `drive_file_id`");
    }
    if (!studentiMovimentiColumnExists('studenti_movimenti_allegati', 'drive_folder_id')) {
        dbExec("ALTER TABLE studenti_movimenti_allegati ADD COLUMN `drive_folder_id` VARCHAR(255) NULL AFTER `drive_web_view_link`");
    }
    ...
    if (!studentiMovimentiColumnExists('studenti_movimenti_eventi', 'allegato_storage_type')) {
        dbExec("ALTER TABLE studenti_movimenti_eventi ADD COLUMN `allegato_storage_type` ENUM('LOCAL','DRIVE') NOT NULL DEFAULT 'LOCAL' AFTER `allegato_original_name`");
    }
    if (!studentiMovimentiColumnExists('studenti_movimenti_eventi', 'allegato_drive_file_id')) {
        dbExec("ALTER TABLE studenti_movimenti_eventi ADD COLUMN `allegato_drive_file_id` VARCHAR(255) NULL AFTER `allegato_storage_type`");
    }
    if (!studentiMovimentiColumnExists('studenti_movimenti_eventi', 'allegato_drive_web_view_link')) {
        dbExec("ALTER TABLE studenti_movimenti_eventi ADD COLUMN `allegato_drive_web_view_link` VARCHAR(500) NULL AFTER `allegato_drive_file_id`");
    }
    if (!studentiMovimentiColumnExists('studenti_movimenti_eventi', 'allegato_drive_folder_id')) {
        dbExec("ALTER TABLE studenti_movimenti_eventi ADD COLUMN `allegato_drive_folder_id` VARCHAR(255) NULL AFTER `allegato_drive_web_view_link`");
    }
...
function studentiMovimentiCompactName($value): string
{
    $value = studentiMovimentiUpperName($value);
...
function studentiMovimentiFindOrCreateStudentForEntrata(array $fields): int
{
    ...
    $compactCognome = studentiMovimentiCompactName($cognome);
    $compactNome = studentiMovimentiCompactName($nome);
    if ($compactCognome !== '' && $compactNome !== '') {
        $studentId = intval(dbGetValue("
            SELECT id
            FROM studente
            WHERE TRIM(COALESCE(codice_fiscale, '')) = ''
              AND UPPER(REPLACE(REPLACE(REPLACE(TRIM(cognome), ' ', ''), '''', ''), '-', '')) = " . dbQ($compactCognome) . "
              AND UPPER(REPLACE(REPLACE(REPLACE(TRIM(nome), ' ', ''), '''', ''), '-', '')) = " . dbQ($compactNome) . "
            ORDER BY id ASC
            LIMIT 1
        ") ?? 0);
        if ($studentId > 0) {
    ...
    $cfGestore = $cf;
    if ($cfGestore === '') {
        $movementId = intval($fields['id'] ?? 0);
        $cfGestore = $movementId > 0
            ? ('MOVENT' . str_pad((string)$movementId, 10, '0', STR_PAD_LEFT))
            : ('MOV' . strtoupper(substr(md5($cognome . '|' . $nome . '|' . (string)($fields['data_nascita'] ?? '')), 0, 13)));
    }
...
function studentiMovimentiParentRelationId(string $tipo): int
{
    if (function_exists('iscrizioniPrimeRelazioneId')) {
        return iscrizioniPrimeRelazioneId($tipo);
    }
...
function studentiMovimentiUpsertGestoreParent(array $parent, int $studentId): int
{
    ...
    if ($studentId <= 0 || ($cf === '' && $email === '') || ($cognome === '' && $nome === '')) {
        return 0;
    }

    $where = $cf !== ''
        ? "UPPER(TRIM(codice_fiscale)) = " . dbQ($cf)
        : "LOWER(TRIM(email)) = " . dbQ($email);

    $parentId = intval(dbGetValue("SELECT id FROM genitori WHERE $where LIMIT 1") ?? 0);
    if ($parentId > 0) {
        $updates = [
            "attivo = 1",
        ];
        if ($cognome !== '') {
            $updates[] = "cognome = " . dbQ($cognome);
        }
        if ($nome !== '') {
            $updates[] = "nome = " . dbQ($nome);
        }
        if ($email !== '') {
            $updates[] = "email = " . dbQNotNull($email);
            $updates[] = "username = " . dbQNotNull($email);
        }
        if ($cf !== '') {
            $updates[] = "codice_fiscale = " . dbQ($cf);
        }

        dbExec("
            UPDATE genitori
            SET " . implode(",\n                ", $updates) . "
            WHERE id = " . dbI($parentId) . "
            LIMIT 1
        ");
    } else {
        $cfForInsert = $cf !== ''
            ? $cf
            : ('MOVPAR' . strtoupper(substr(md5($studentId . '|' . $email . '|' . $cognome . '|' . $nome), 0, 10)));
        dbExec("
            INSERT INTO genitori (cognome, nome, email, codice_fiscale, username, attivo, last_login, last_IP)
            VALUES (" . dbQNotNull($cognome) . ", " . dbQNotNull($nome) . ", " . dbQNotNull($email) . ", " . dbQNotNull($cfForInsert) . ", " . dbQNotNull($email) . ", 1, '', '')
        ");
        $parentId = intval(dblastId());
    }

    $relationId = studentiMovimentiParentRelationId((string)($parent['tipo'] ?? 'genitore'));
    if ($parentId > 0 && $relationId > 0) {
        dbExec("
            INSERT INTO genitori_studenti (id_genitore, id_studente, id_relazione)
            VALUES (" . dbI($parentId) . ", " . dbI($studentId) . ", " . dbI($relationId) . ")
...
function studentiMovimentiSyncParentsForEntrata(array $fields): array
{
    if (($fields['tipo_pratica'] ?? '') !== 'entrata') {
        return ['read' => 0, 'linked' => 0, 'ids' => []];
    }

    $studentId = intval($fields['id_studente'] ?? 0);
    if ($studentId <= 0) {
        return ['read' => 0, 'linked' => 0, 'ids' => []];
    }
    ...
    $parents = [
        [
            'tipo' => $fields['responsabile_1_tipo'] ?? 'genitore',
            'cognome' => $fields['responsabile_1_cognome'] ?? '',
            'nome' => $fields['responsabile_1_nome'] ?? '',
            'codice_fiscale' => $fields['responsabile_1_codice_fiscale'] ?? '',
            'email' => $fields['email_genitore_1'] ?? '',
        ],
        [
            'tipo' => $fields['responsabile_2_tipo'] ?? 'genitore',
            'cognome' => $fields['responsabile_2_cognome'] ?? '',
            'nome' => $fields['responsabile_2_nome'] ?? '',
            'codice_fiscale' => $fields['responsabile_2_codice_fiscale'] ?? '',
            'email' => $fields['email_genitore_2'] ?? '',
        ],
    ];

    $ids = [];
    foreach ($parents as $parent) {
        $parentId = studentiMovimentiUpsertGestoreParent($parent, $studentId);
        if ($parentId > 0) {
            $ids[] = $parentId;
        }
    }
    ...
    if ($ids) {
        info('[movimenti] sincronizzati genitori GestOre per entrata studente id=' . $studentId . ' genitori=' . implode(',', $ids));
    }

    return ['read' => count($parents), 'linked' => count($ids), 'ids' => $ids];
}
...
function studentiMovimentiEnsureStudentsForEntrate(): array
{
    ...
    foreach ($rows as $row) {
        try {
            ...
            dbExec("
                ...
                LIMIT 1
            ");
            $row['id_studente'] = $studentId;
            studentiMovimentiSyncParentsForEntrata($row);
            $linked++;
        } catch (Throwable $e) {
            $skipped++;
            if (count($errors) < 20) {
    ...
    $parentRows = dbGetAll("
        ...
        WHERE tipo_pratica = 'entrata'
          ...
          AND (
              ...
              OR TRIM(COALESCE(responsabile_1_codice_fiscale, '')) <> ''
              OR TRIM(COALESCE(email_genitore_2, '')) <> ''
              OR TRIM(COALESCE(responsabile_2_codice_fiscale, '')) <> ''
          )
        ORDER BY id ASC
    ") ?: [];
    $parentsLinked = 0;
    foreach ($parentRows as $row) {
        try {
            $parentSync = studentiMovimentiSyncParentsForEntrata($row);
            $parentsLinked += intval($parentSync['linked'] ?? 0);
        } catch (Throwable $e) {
            if (count($errors) < 20) {
                $errors[] = trim((string)($row['cognome'] ?? '') . ' ' . (string)($row['nome'] ?? '')) . ' genitori: ' . $e->getMessage();
            }
        }
    }

    return ['read' => count($rows), 'linked' => $linked, 'activated' => $inactiveLinked, 'parents_linked' => $parentsLinked, 'skipped' => $skipped, 'errors' => $errors];
}
...
function studentiMovimentiRiepilogoIscrittiReali(): array
{
    iscrizioniPrimeEnsureSchema();
    studentiMovimentiEnsureTables();

    $currentYearId = studentiMovimentiCurrentYearId();
    $summary = [];
    for ($year = 1; $year <= 5; $year++) {
        $summary[$year] = [
            'anno' => $year,
            'label' => studentiMovimentiYearLabel($year),
            'base_label' => in_array($year, [1, 3], true) ? 'Pratiche iniziali' : 'Iscritti attuali anno precedente',
            'base' => 0,
            'in_arrivo' => 0,
            'usciti' => 0,
            'doppi_bocciati' => 0,
    ...
    foreach ([1 => 'prime', 3 => 'terze'] as $year => $tipoIscrizione) {
        $schoolYear = studentiMovimentiSchoolYearForIscrizione($tipoIscrizione);
        $rows = dbGetAll("
            SELECT id, id_studente_gestore, cognome, nome, codice_fiscale
            FROM iscrizioni_prime_pratiche p
            WHERE p.tipo_iscrizione = " . dbQ($tipoIscrizione) . "
              AND (p.stato IS NULL OR p.stato <> 'annullata')
              AND " . iscrizioniPrimeSchoolYearWhere('p.anno_scolastico', $schoolYear) . "
        ") ?: [];
        foreach ($rows as $row) {
            $key = studentiMovimentiStudentKey($row);
            if ($key !== '') {
                $summary[$year]['base_keys'][$key] = true;
            }
        }
    }
    ...
    if ($currentYearId > 0) {
        $rows = dbGetAll("
            SELECT s.id AS id_studente, s.cognome, s.nome, s.codice_fiscale, c.classe
            FROM studente s
            INNER JOIN studente_frequenta sf ON sf.id_studente = s.id
              AND sf.id_anno_scolastico = " . dbI($currentYearId) . "
            INNER JOIN classi c ON c.id = sf.id_classe
            WHERE COALESCE(s.attivo, 1) = 1
        ") ?: [];
        foreach ($rows as $row) {
            $currentClassYear = mastercomTabelloniClassYearFromName((string)($row['classe'] ?? ''));
            $targetYear = $currentClassYear + 1;
            if (!in_array($targetYear, [2, 4, 5], true)) {
                continue;
            }
            $key = studentiMovimentiStudentKey($row);
            if ($key !== '') {
                $summary[$targetYear]['base_keys'][$key] = true;
            }
        }
    }
    ...
    $practiceRows = dbGetAll("
        SELECT
            ...
            linked.id AS linked_pratica_id
        FROM studenti_movimenti_pratiche p
        LEFT JOIN studente s ON s.id = p.id_studente
        LEFT JOIN iscrizioni_prime_pratiche linked ON linked.id = p.id_pratica_iscrizione
          AND (linked.stato IS NULL OR linked.stato <> 'annullata')
        WHERE p.stato_pratica <> 'annullata'
          AND COALESCE(p.anno_corso, 0) BETWEEN 1 AND 5
    ") ?: [];
    ...
    foreach ($practiceRows as $row) {
        ...
        $key = studentiMovimentiStudentKey([
            'id_studente' => $row['id_studente'] ?? $row['studente_id'] ?? null,
            'codice_fiscale' => ($row['codice_fiscale'] ?? '') ?: ($row['studente_cf'] ?? ''),
            'cognome' => ($row['cognome'] ?? '') ?: ($row['studente_cognome'] ?? ''),
            'nome' => ($row['nome'] ?? '') ?: ($row['studente_nome'] ?? ''),
        ]);
        if ($key === '') {
            continue;
        }
        ...
        $state = (string)($row['stato_pratica'] ?? '');
        if ($type === 'entrata') {
            $linkedInitialPractice = intval($row['linked_pratica_id'] ?? 0) > 0 && in_array($year, [1, 3], true);
            if (!$linkedInitialPractice && studentiMovimentiIncomingCountsInRealSummary($state)) {
                $summary[$year]['incoming_keys'][$key] = true;
            }
            continue;
        }

        if (!empty($row['doppio_bocciato'])) {
            $summary[$year]['double_keys'][$key] = true;
        }
        if (studentiMovimentiOutgoingCountsInRealSummary($row)) {
            $summary[$year]['outgoing_keys'][$key] = true;
        }
    }

    foreach ($summary as $year => &$row) {
        $baseKeys = array_keys($row['base_keys']);
        $incomingKeys = array_keys($row['incoming_keys']);
        $outgoingKeys = array_keys($row['outgoing_keys']);
        $row['base'] = count($baseKeys);
        $row['in_arrivo'] = count($incomingKeys);
        $row['usciti'] = count($outgoingKeys);
        $row['doppi_bocciati'] = count($row['double_keys']);
        $row['totale_reale'] = count(array_unique(array_merge($baseKeys, $incomingKeys))) - $row['usciti'];
        unset($row['base_keys'], $row['incoming_keys'], $row['outgoing_keys'], $row['double_keys']);
    }
    unset($row);
...
function studentiMovimentiIncomingCountsInRealSummary(string $state): bool
{
    return !in_array($state, ['annullata', 'non_idoneo'], true);
}

function studentiMovimentiOutgoingCountsInRealSummary(array $row): bool
{
    if (!empty($row['doppio_bocciato'])) {
        return true;
    }
    $type = (string)($row['tipo_pratica'] ?? '');
    $state = (string)($row['stato_pratica'] ?? '');
    if (!in_array($type, ['uscita', 'ritiro'], true)) {
        return false;
    }
    if ($state === 'annullata') {
        return false;
    ...
    if ($type === 'ritiro') {
        return in_array($state, ['si_ritira', 'firmato_un_genitore', 'firmato_entrambi', 'colloquio_richiesto', 'colloquio_da_programmare', 'colloquio_programmato', 'colloquio_uscita', 'chiusa'], true);
    }
    return in_array($state, ['cambia_scuola', 'richiesta_nulla_osta', 'firmato_un_genitore', 'firmato_entrambi', 'colloquio_richiesto', 'colloquio_da_programmare', 'colloquio_programmato', 'colloquio_uscita', 'nulla_osta_inviato', 'chiusa'], true);
}

function studentiMovimentiStudentKey(array $row): string
{
    $studentId = intval($row['id_studente'] ?? $row['id_studente_gestore'] ?? $row['studente_id'] ?? 0);
    if ($studentId > 0) {
        return 'id:' . $studentId;
    }
    $cf = strtoupper(trim((string)($row['codice_fiscale'] ?? $row['studente_cf'] ?? '')));
    if ($cf !== '') {
        return 'cf:' . $cf;
    }
    $name = studentiMovimentiUpperName(trim((string)($row['cognome'] ?? $row['studente_cognome'] ?? '') . ' ' . (string)($row['nome'] ?? $row['studente_nome'] ?? '')));
    return $name !== '' ? 'name:' . preg_replace('/\s+/', ' ', $name) : '';
}

function studentiMovimentiYearLabel(int $year): string
{
...
function studentiMovimentiArchiveDuplicateIscrizioniForEntrata(int $primaryPracticeId, array $fields, string $tipoIscrizione): void
{
    if ($primaryPracticeId <= 0 || !in_array($tipoIscrizione, ['prime', 'terze'], true)) {
        return;
    }
    $cf = strtoupper(trim((string)($fields['codice_fiscale'] ?? '')));
    if ($cf === '') {
    ...
    $primary = dbGetFirst("
        ...
        FROM iscrizioni_prime_pratiche
        WHERE id = " . dbI($primaryPracticeId) . "
        LIMIT 1
    ") ?: [];
    $raw = json_decode((string)($primary['raw_prime_json'] ?? ''), true);
    $primaryGenerated = is_array($raw) && (($raw['FONTE'] ?? '') === 'movimenti_entrata');

    $duplicates = dbGetAll("
        SELECT id, stato, raw_prime_json
        FROM iscrizioni_prime_pratiche
        WHERE id <> " . dbI($primaryPracticeId) . "
          AND tipo_iscrizione = " . dbQ($tipoIscrizione) . "
          AND UPPER(TRIM(codice_fiscale)) = " . dbQ($cf) . "
          AND stato IN ('importata', 'bozza')
        ORDER BY updated_at DESC, id DESC
    ") ?: [];
    foreach ($duplicates as $duplicate) {
        $duplicateId = intval($duplicate['id'] ?? 0);
        if ($duplicateId <= 0) {
            continue;
        }
        $duplicateRaw = json_decode((string)($duplicate['raw_prime_json'] ?? ''), true);
        $duplicateGenerated = is_array($duplicateRaw) && (($duplicateRaw['FONTE'] ?? '') === 'movimenti_entrata');
        if (!$primaryGenerated && !$duplicateGenerated) {
            continue;
        }
        $sentMails = intval(dbGetValue("
            SELECT COUNT(*)
            FROM iscrizioni_prime_mail_log
            WHERE pratica_id = " . dbI($duplicateId) . "
        ...
        dbExec("
            UPDATE studenti_movimenti_pratiche
            SET id_pratica_iscrizione = " . dbI($primaryPracticeId) . ",
                updated_at = NOW()
            WHERE id_pratica_iscrizione = " . dbI($duplicateId) . "
        ");
        dbExec("
            UPDATE iscrizioni_prime_pratiche
            SET stato = 'annullata',
                note_interne = CONCAT(COALESCE(note_interne, ''), '\nPratica archiviata automaticamente: duplicata da entrata movimenti collegata alla pratica #" . intval($primaryPracticeId) . ".'),
                updated_at = NOW()
            WHERE id = " . dbI($duplicateId) . "
            LIMIT 1
        ");
    }
}

function studentiMovimentiArchiveIscrizioniForNonInitialEntrata(array $fields, int $movementId = 0): void
{
    $cf = strtoupper(trim((string)($fields['codice_fiscale'] ?? '')));
    if ($cf === '') {
        return;
    ...
    iscrizioniPrimeEnsureSchema();
    $rows = dbGetAll("
        SELECT id, stato, tipo_iscrizione
        FROM iscrizioni_prime_pratiche
        WHERE UPPER(TRIM(codice_fiscale)) = " . dbQ($cf) . "
          AND tipo_iscrizione IN ('prime', 'terze')
          AND stato IN ('importata', 'bozza')
        ORDER BY updated_at DESC, id DESC
    ") ?: [];
    foreach ($rows as $row) {
        $practiceId = intval($row['id'] ?? 0);
        if ($practiceId <= 0) {
            continue;
        }
        $sentMails = intval(dbGetValue("
        ...
        dbExec("
            UPDATE iscrizioni_prime_pratiche
            SET stato = 'annullata',
                note_interne = CONCAT(COALESCE(note_interne, ''), '\nPratica archiviata automaticamente: movimento entrata in anno " . intval($fields['anno_corso'] ?? 0) . ", quindi non e una iscrizione iniziale prime/terze. Movimento #" . intval($movementId) . ".'),
                updated_at = NOW()
            WHERE id = " . dbI($practiceId) . "
            LIMIT 1
        ");
        iscrizioniPrimeRecordEvent($practiceId, 'archiviazione_movimento_entrata', 'Pratica archiviata per entrata in altro anno', [
            'stato_precedente' => (string)($row['stato'] ?? ''),
            'stato_nuovo' => 'annullata',
            'messaggio' => 'Lo studente risulta in entrata in anno ' . intval($fields['anno_corso'] ?? 0) . ': non deve stare nelle iscrizioni prime/terze.',
            'dettagli' => [
                'movimento_entrata_id' => $movementId,
                'anno_corso_movimento' => intval($fields['anno_corso'] ?? 0),
                'codice_fiscale' => $cf,
            ],
        ]);
    }
...
function studentiMovimentiEnsureIscrizioneForEntrata(array &$fields, int $movementId = 0): void
{
    ...
    if (!in_array($tipoIscrizione, ['prime', 'terze'], true)) {
        ...
        studentiMovimentiArchiveIscrizioniForNonInitialEntrata($fields, $movementId);
        return;
    }
    if (intval($fields['id_pratica_iscrizione'] ?? 0) > 0) {
        iscrizioniPrimeEnsureSchema();
        $linked = dbGetFirst("
            SELECT id, anno_scolastico, codice_fiscale, stato, raw_prime_json
            FROM iscrizioni_prime_pratiche
            WHERE id = " . dbI($fields['id_pratica_iscrizione']) . "
            LIMIT 1
        ") ?: [];
        $raw = json_decode((string)($linked['raw_prime_json'] ?? ''), true);
        $linkedGenerated = is_array($raw) && (($raw['FONTE'] ?? '') === 'movimenti_entrata');
        $currentCf = strtoupper(trim((string)($fields['codice_fiscale'] ?? '')));
        $linkedCf = strtoupper(trim((string)($linked['codice_fiscale'] ?? '')));
        if ($linkedGenerated && $currentCf !== '' && $linkedCf !== '' && $currentCf !== $linkedCf) {
            $annoScolastico = studentiMovimentiSchoolYearForIscrizione($tipoIscrizione);
            $correctPractice = dbGetFirst("
                SELECT id
                FROM iscrizioni_prime_pratiche
                WHERE " . iscrizioniPrimeSchoolYearWhere('anno_scolastico', $annoScolastico) . "
                  AND tipo_iscrizione = " . dbQ($tipoIscrizione) . "
                  AND UPPER(TRIM(codice_fiscale)) = " . dbQ($currentCf) . "
                  AND stato <> 'annullata'
                ORDER BY updated_at DESC, id DESC
                LIMIT 1
            ");
            $correctPracticeId = intval($correctPractice['id'] ?? 0);
            if ($correctPracticeId > 0) {
                dbExec("
                    UPDATE iscrizioni_prime_pratiche
                    SET stato = 'annullata',
                        note_interne = CONCAT(COALESCE(note_interne, ''), '\nPratica archiviata automaticamente: generata da movimento entrata con codice fiscale precedente " . dbEscape($linkedCf) . ", ricollegata alla pratica #" . intval($correctPracticeId) . ".'),
                        updated_at = NOW()
                    WHERE id = " . dbI($fields['id_pratica_iscrizione']) . "
                    LIMIT 1
                ");
                iscrizioniPrimeRecordEvent(intval($fields['id_pratica_iscrizione']), 'archiviazione_doppione', 'Pratica movimenti archiviata per CF corretto', [
                    'stato_precedente' => (string)($linked['stato'] ?? ''),
                    'stato_nuovo' => 'annullata',
                    'messaggio' => 'Il movimento entrata ora usa il codice fiscale ' . $currentCf . ' ed e stato ricollegato alla pratica #' . $correctPracticeId . '.',
                    'dettagli' => [
                        'codice_fiscale_precedente' => $linkedCf,
                        'codice_fiscale_corretto' => $currentCf,
                        'pratica_corretta_id' => $correctPracticeId,
                        'movimento_entrata_id' => $movementId,
                    ],
                ]);
                $fields['id_pratica_iscrizione'] = $correctPracticeId;
                if ($movementId > 0) {
                    dbExec("
                        UPDATE studenti_movimenti_pratiche
                        SET id_pratica_iscrizione = " . dbI($correctPracticeId) . ",
                            updated_at = NOW()
                        WHERE id = " . dbI($movementId) . "
                        LIMIT 1
                    ");
                }
                $linked = dbGetFirst("SELECT id, raw_prime_json FROM iscrizioni_prime_pratiche WHERE id = " . dbI($correctPracticeId) . " LIMIT 1") ?: [];
                $raw = json_decode((string)($linked['raw_prime_json'] ?? ''), true);
                $linkedGenerated = is_array($raw) && (($raw['FONTE'] ?? '') === 'movimenti_entrata');
            }
        }
        studentiMovimentiUpdateIscrizioneContactsFromEntrata(intval($fields['id_pratica_iscrizione']), $fields, $linkedGenerated);
        studentiMovimentiSyncGeneratedIscrizioneDocuments($movementId, intval($fields['id_pratica_iscrizione']));
        studentiMovimentiArchiveDuplicateIscrizioniForEntrata(intval($fields['id_pratica_iscrizione']), $fields, $tipoIscrizione);
        return;
    }
    ...
    if ($existing) {
        ...
        studentiMovimentiArchiveDuplicateIscrizioniForEntrata(intval($existing['id']), $fields, $tipoIscrizione);
        return;
    }
    ...
    studentiMovimentiArchiveDuplicateIscrizioniForEntrata($praticaId, $fields, $tipoIscrizione);
}
...
function studentiMovimentiEnsureIscrizioniForEntrate(): array
{
    ...
    $rows = dbGetAll("
        ...
        WHERE tipo_pratica = 'entrata'
          AND stato_pratica <> 'annullata'
          AND COALESCE(anno_corso, 0) BETWEEN 1 AND 5
        ORDER BY id ASC
    ") ?: [];
    ...
    foreach ($rows as $row) {
        ...
        try {
            ...
            if ($praticaId > 0) {
                ...
                if (intval($row['id_pratica_iscrizione'] ?? 0) > 0) {
                    dbExec("
                        UPDATE studenti_movimenti_pratiche
                        SET id_pratica_iscrizione = NULL,
                            updated_at = NOW()
                        WHERE id = " . dbI($row['id'] ?? 0) . "
                        LIMIT 1
                    ");
...
function studentiMovimentiDriveEnabled(): bool
{
    try {
...
function studentiMovimentiDriveFolderId(int $practiceId): string
{
    require_once __DIR__ . '/../api/googleDriveLib.php';
    $practice = dbGetFirst("SELECT * FROM studenti_movimenti_pratiche WHERE id = " . dbI($practiceId) . " LIMIT 1") ?: [];
    $rootId = googleDriveGetConfiguredFolderId('movimentiFolderId', 'movimentiFolderName', 'Movimenti Entrate-Uscite');
    $schoolYear = studentiMovimentiSchoolYearLabel();
    $yearFolderId = googleDriveGetOrCreateFolderInParent(iscrizioniPrimeDriveSafeName($schoolYear), $rootId);
    $type = iscrizioniPrimeDriveSafeName((string)($practice['tipo_pratica'] ?? 'pratica'));
    $typeFolderId = googleDriveGetOrCreateFolderInParent($type !== '' ? $type : 'pratica', $yearFolderId);
    $name = trim((string)($practice['cognome'] ?? '') . ' ' . (string)($practice['nome'] ?? ''));
    $cf = trim((string)($practice['codice_fiscale'] ?? ''));
    $folderName = $name !== '' ? $name : ('Pratica ' . $practiceId);
    if ($cf !== '') {
        $folderName .= ' - ' . $cf;
    }
    $folderName .= ' - pratica ' . $practiceId;
    return googleDriveGetOrCreateFolderInParent(iscrizioniPrimeDriveSafeName($folderName), $typeFolderId);
}

function studentiMovimentiSchoolYearLabel(): string
{
    $year = intval(date('Y'));
    $month = intval(date('n'));
...
function studentiMovimentiUploadMetadata(int $practiceId, string $localPath, string $original, string $mimeType = ''): array
{
    $meta = [
        'storage_type' => 'LOCAL',
        'drive_file_id' => null,
    ...
    if (!studentiMovimentiDriveEnabled()) {
        return $meta;
    }
...
function studentiMovimentiSavePractice(array $data): int
{
    ...
    if ($id > 0) {
        ...
        if ($fields['tipo_pratica'] === 'entrata') {
            studentiMovimentiSyncParentsForEntrata($fields);
        }
...
function studentiMovimentiAddEvent(int $practiceId, string $type, string $description, array $fields = [], string $createdBy = ''): int
{
    if ($practiceId <= 0) {
        return 0;
    }
    ...
    if ($type === 'colloquio_genitori' && $linkedColloquioId > 0) {
        ...
        if ($existingEventId > 0) {
            $nextValues = [
                ...
                'allegato_storage_type' => $fields['allegato_storage_type'] ?? 'LOCAL',
                'allegato_drive_file_id' => $fields['allegato_drive_file_id'] ?? null,
                'allegato_drive_web_view_link' => $fields['allegato_drive_web_view_link'] ?? null,
                'allegato_drive_folder_id' => $fields['allegato_drive_folder_id'] ?? null,
            ...
            dbExec("
                ...
                SET descrizione = " . dbQ($description) . ",
                    ...
                    allegato_storage_type = " . dbQ($fields['allegato_storage_type'] ?? 'LOCAL') . ",
                    allegato_drive_file_id = " . dbQ($fields['allegato_drive_file_id'] ?? null) . ",
                    allegato_drive_web_view_link = " . dbQ($fields['allegato_drive_web_view_link'] ?? null) . ",
                    allegato_drive_folder_id = " . dbQ($fields['allegato_drive_folder_id'] ?? null) . ",
    ...
    dbExec("
        INSERT INTO studenti_movimenti_eventi (
            ...
            allegato_storage_type, allegato_drive_file_id, allegato_drive_web_view_link, allegato_drive_folder_id,
            ...
            " . dbQ($fields['allegato_storage_type'] ?? 'LOCAL') . ",
            " . dbQ($fields['allegato_drive_file_id'] ?? null) . ",
            " . dbQ($fields['allegato_drive_web_view_link'] ?? null) . ",
            " . dbQ($fields['allegato_drive_folder_id'] ?? null) . ",
...
{
    $ids = array_values(array_filter(array_map('intval', $practiceIds), static function ($id) {
        return $id > 0;
    }));
    if (!$ids) {
        return [];
...
function studentiMovimentiAttachFileToEvent(int $eventId, array $file, string $type): bool
{
    ...
    $storage = studentiMovimentiUploadMetadata($practiceId, $target, $original, (string)($file['type'] ?? ''));

    dbExec("
        INSERT INTO studenti_movimenti_allegati (
            id_pratica, id_evento, tipo_allegato, nome_file, path_file, mime_type, dimensione,
            storage_type, drive_file_id, drive_web_view_link, drive_folder_id, created_at
        ) VALUES (
            " . dbI($practiceId) . ",
            " . dbI($eventId) . ",
            " . dbQ($type) . ",
...
function studentiMovimentiDeleteAttachment(int $attachmentId): bool
{
    ...
    if (strtoupper((string)($attachment['storage_type'] ?? 'LOCAL')) === 'DRIVE' && trim((string)($attachment['drive_file_id'] ?? '')) !== '') {
        try {
            require_once __DIR__ . '/../api/googleDriveLib.php';
            googleDriveDeleteFile((string)$attachment['drive_file_id']);
        } catch (Throwable $e) {
            // Il record locale va comunque rimosso; l'eventuale pulizia Drive si potra ripetere manualmente.
        }
    }
...
function studentiMovimentiAttachFile(int $practiceId, array $file, string $type): void
{
    ...
    dbExec("
        INSERT INTO studenti_movimenti_allegati (
            id_pratica, tipo_allegato, nome_file, path_file, mime_type, dimensione,
            storage_type, drive_file_id, drive_web_view_link, drive_folder_id, created_at
        ) VALUES (
            " . dbI($practiceId) . ",
            " . dbQ($type) . ",
