...
try {
    if ($_SERVER['REQUEST_METHOD'] === 'POST') {
        ...
        if (isset($_POST['action']) && $_POST['action'] === 'process') {
            ...
            $tickets = extractTicketsFromMultiPagePdf($pdfPath);
        ...
        if (!empty($_SESSION['tickets_last_result'])) {
            $result = $_SESSION['tickets_last_result'];
        } else {
        ...
        <div class="box">
            ...
            <div class="left-summary-stats">
                <div class="stat">
                    ...
                    <div class="k">Biglietti letti</div>
                </div>
                <div class="stat">
            ...
            <div class="box">
                ...
                <div class="tribuna-wrap">
                    <div class="tribuna-title">
                        <?= h((string)($result['tribuna'] ?? 'Tribuna')) ?>
                        <span class="pill"><?= (int)($result['row_count'] ?? 10) ?> file · <?= (int)($result['seat_count'] ?? 54) ?> posti</span>
                    </div>
                    ...
                    $seatCount = (int)($result['seat_count'] ?? 54);
                    $seatMapIndexed = [];
                    foreach (($result['seat_map'] ?? []) as $cell) {
                        $seatMapIndexed[$cell['fila'] . '-' . $cell['posto']] = $cell;
                    }
                    ?>

                    <div class="seat-grid" style="grid-template-columns:66px repeat(<?= $seatCount ?>, minmax(24px, 1fr)); min-width: <?= 66 + ($seatCount * 28) ?>px;">
                        ...
                        <?php for ($fila = 1; $fila <= $rowCount; $fila++): ?>
                            ...
                            <?php for ($seat = 1; $seat <= $seatCount; $seat++): ?>
                                <?php $k = $fila . '-' . $seat; $cell = $seatMapIndexed[$k] ?? null; ?>
                                <?php if ($cell): ?>
                                    <?php
                                    $classes = ['seat'];
                                    ...
                                    ><?= $seat ?></div>
                                <?php else: ?>
                                    <div class="seat no-ticket" data-tooltip="Fila <?= $fila ?> - Posto <?= $seat ?> - NESSUN BIGLIETTO"><?= $seat ?></div>
                                <?php endif; ?>
                            <?php endfor; ?>
                        <?php endfor; ?>
                    </div>
