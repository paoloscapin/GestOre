...
function registraVotoSettembre(studente_per_corso_di_recupero_id, voto, __this) {
	$.post("corsoDiRecuperoRegistraVoto.php", {
		...
		function (data, status) {
			...
			if (__this.value > 5) {
				$('td:nth-child(10)', $(__this).parents('tr')).html('<span class=\'label label-success\' style=\'background-color: Green;>passato</span>');
				...
				$('td:nth-child(10)', $(__this).parents('tr')).html('<span class=\'label label-danger\' style=\'background-color: FireBrick;>non passato</span>');
				// $('td:nth-child(11)', $(__this).parents('tr')).html('<button onclick="letteraCarenzeSettembre('+studente_per_corso_di_recupero_id+')" class="btn btn-success btn-xs"><span class="glyphicon glyphicon-envelope"></button>');
			}
		}
	);
...
function registraDocenteSettembre(studente_per_corso_di_recupero_id, docente_id, docente_incaricato_cognomenome, __this) {
	$.post("corsoDiRecuperoRegistraDocente.php", {
		...
		function (data, status) {
			...
			$('td:nth-child(6)', $(__this).parents('tr')).html('<small>' + docente_incaricato_cognomenome + '</small>');
		}
	);
}
...
$(document).ready(function () {
	...
	$(".votoSettembre").on('change', function(e){
			...
			registraDataVoto(studente_per_corso_di_recupero_id, Date.today().toString('yyyy-MM-dd'),'data_voto_settembre');
			$('td:nth-child(5)', $(this).parents('tr')).children(":first").val(Date.today().toString('dd/MM/yyyy'));
		});
