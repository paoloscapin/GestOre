...
/**
 * Carenze (studente) - esiti corretti con gestione:
...
function renderEsame($label, $s, $extraPrefixHtml = '')
{
    ...
    // firmato: esito studente presente/assente
    ...
    if ($presente === 0) {
        // ASSENTE => niente "recuperato/non recuperato"
        $html .= ($assG === 1)
    ...
    // PRESENTE => mostra recuperato/non recuperato
...
foreach ($carenze as $row) {
    ...
    // ===========================
    // 1) Trovo il corso di 1ª sessione (sessione=1) per materia/anno e studente
    // ===========================
    $idCorso1 = dbGetValue("
        SELECT co.id
        FROM corso co
    ...
    // Esame del corso1 tentativo=1 + esito studente
    $primo = dbGetFirst("
        SELECT
    ...
    // ===========================
    // 2) Cerco eventuale corso2 collegato dal mapping
    // ===========================
    $map = dbGetFirst("
        SELECT ccs.id_corso_secondo, co2.titolo
        FROM corso_carenze_seconda ccs
        INNER JOIN corso co1 ON co1.id = ccs.id_corso_primo
    ...
    if ($map && intval($map['id_corso_secondo']) > 0) {
        $hasSecondo = true;
    }
