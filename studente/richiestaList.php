...
<table id="manno" class="table table-bordered table-striped table-green">
    ...
    <tbody><tr>
        <td>Rappresentante di clase:</td><td></td>
    </tr></tbody>
</table>
</div>
...
foreach(dbGetAll("SELECT * FROM modulistica_categoria ORDER BY posizione;") as $categoria) {
    ...
    foreach(dbGetAll("SELECT * FROM modulistica_template WHERE modulistica_template.valido = true AND modulistica_template.modulistica_categoria_id = $categoriaId ORDER BY posizione;") as $template) {
        ...
        echo '<tr>';
        echo '<td><a href="../docente/modulisticaCompilaModulo.php?studente_id='.$studente_id.'&template_id='.$template_id.'" target="'.$openTabMode.'">&ensp;'.$templateNome.' '.$marker.' </a></td>';
    }
}
?>
        </tbody>
