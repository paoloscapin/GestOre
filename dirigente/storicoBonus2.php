...
foreach(dbGetAll("SELECT * FROM docente ORDER BY docente.cognome, docente.nome ASC ;") as $docente) {
	...
	if (!empty($assegnatoList)) {
		...
		$data .= '<table><thead><tr><th class="col-sd-11">Commento</th><th class="text-center col-sd-1">Importo</th></tr></thead><tbody>';
	...
	$data .='<table>';
	$data .='<thead><tr><th class="text-center col-sd-1">Codice</th><th class="text-center col-sd-8">Descrittore</th><th class="text-center col-sd-1">Valore</th><th class="text-center col-sd-1">Approvato</th><th class="text-center col-sd-1">Importo</th></tr>';
	$data .='</thead><tbody>';
...
$dataConsuntivo .= '<tr><td class="col-sd-11 text-left">Totale Bonus Assegnato</td><td class="col-sd-1 text-right">' . number_format($totaleAssegnatoIstuto,2) . '</td></tr>';
$dataConsuntivo .= '<tr><td class="col-sd-11 text-left">Totale Bonus Approvato</td><td class="col-sd-1 text-right">' . number_format($totaleApprovatoIstuto,2) . '</td></tr>';
$dataConsuntivo .= '</tbody><tfoot>';
$dataConsuntivo .='<tr><td colspan="1" class="text-right"><strong>Totale:</td><td class="text-right">' . formatNoZero($totaleAssegnatoIstuto + $totaleApprovatoIstuto) . '</strong></td></tr>';
...
// titolo
$title = 'Storico Bonus ' . $nome_anno_scolastico.' - '.getSettingsValue('local','nomeIstituto', '');
