...
function mastercomAbsenceCreateRomeNow(): DateTime
{
    return new DateTime('now', new DateTimeZone('Europe/Rome'));
}

function mastercomAbsenceCreateClean($value): string
{
    return trim((string)(mastercomAdminCleanText($value) ?? ''));
}

function mastercomAbsenceCreateStudent(int $studentId): ?array
{
...
function mastercomAbsenceCreateClassName(int $classId): string
{
    if ($classId <= 0 || !mastercomAdminTableExists('mastercom_classi')) {
        return '';
    }

    return mastercomAbsenceCreateClean(dbGetValue("SELECT nome FROM mastercom_classi WHERE mastercom_id_classe = " . intval($classId) . " LIMIT 1") ?? '');
}

function mastercomAbsenceCreateDateParts(string $date): array
{
    $dt = DateTime::createFromFormat('Y-m-d', $date, new DateTimeZone('Europe/Rome'));
    if (!$dt instanceof DateTime) {
        $dt = mastercomAbsenceCreateRomeNow();
    }
...
function mastercomAbsenceCreateTimeParts(string $time): array
{
    if (!preg_match('/^(\d{1,2}):(\d{2})$/', trim($time), $matches)) {
        $now = mastercomAbsenceCreateRomeNow();
        return [
...
$studentRow = mastercomAbsenceCreateStudent($studentId);

if ($studentRow != null) {
    if ($classId <= 0) {
        $classId = intval($studentRow['mastercom_id_classe_corrente'] ?? 0);
    }
    $studentSurname = mastercomAbsenceCreateClean($studentRow['cognome'] ?? '');
    $studentName = mastercomAbsenceCreateClean($studentRow['nome'] ?? '');
} else {
    $studentSurname = mastercomAbsenceCreateClean($_GET['cognome'] ?? $_POST['cognome'] ?? '');
    $studentName = mastercomAbsenceCreateClean($_GET['nome'] ?? $_POST['nome'] ?? '');
}

$className = mastercomAbsenceCreateClassName($classId);
if ($className === '') {
    $className = mastercomAbsenceCreateClean($_GET['classe'] ?? $_POST['classe'] ?? '');
}

$now = mastercomAbsenceCreateRomeNow();
$selectedDate = trim((string)($_POST['absence_date'] ?? $_GET['absence_date'] ?? $now->format('Y-m-d')));
$selectedTime = trim((string)($_POST['absence_time'] ?? $_GET['absence_time'] ?? $now->format('H:i')));
$submitOk = false;
$submitError = '';
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && ($_POST['submit_absence'] ?? '') === '1') {
        ...
        if (!$authResult['ok']) {
            ...
            $payload = array_merge(
                [
                    ...
                    'tipo_giustificazione' => intval($_POST['tipo_giustificazione'] ?? 1),
                    'esclusione_calcolo_monteore' => intval($_POST['esclusione_calcolo_monteore'] ?? 0),
                    'motivazione' => mastercomAbsenceCreateClean($_POST['motivazione'] ?? ''),
                    'x' => '17',
                    'y' => '20',
                ...
                mastercomAbsenceCreateTimeParts($selectedTime),
                mastercomAbsenceCreateDateParts($selectedDate)
            );
            ...
            if ($submitResult['ok']) {
                ...
                $submitError = 'Invio assenza MasterCom fallito: ' . ($submitResult['error'] ?? 'SUBMIT_FAILED');
            }
        }
    }
}
...
<html>
<head>
    <title>MasterCom Nuova Assenza</title>
    <meta charset="UTF-8">
    <?php if ($submitOk): ?>
...
<div class="container-fluid">
    <?php if ($submitOk): ?>
        <div class="mc-success-center">
            ...
            <h2>Assenza inserita correttamente</h2>
    ...
    <div class="panel panel-teal4">
        <div class="panel-heading"><span class="glyphicon glyphicon-calendar"></span>&emsp;Nuova assenza MasterCom</div>
        <div class="panel-body">
            ...
            <?php if ($studentId <= 0 || $classId <= 0): ?>
                <div class="alert alert-warning">Parametri studente o classe mancanti.</div>
            <?php else: ?>
                ...
                <form method="post" action="mastercom_absence_create.php" class="form-horizontal">
                    ...
                    <div class="form-group">
                        ...
                        <div class="col-sm-5">
                            <input type="text" class="form-control" name="motivazione" id="motivazione" value="Motivi famigliari">
                        </div>
                    </div>

                    <div class="form-group">
                        <div class="col-sm-offset-2 col-sm-10">
                            <button type="submit" class="btn btn-primary" onclick="return confirm('Confermi inserimento assenza su MasterCom?');">
                                <span class="glyphicon glyphicon-ok"></span> Invia a MasterCom
                            </button>
                        </div>
                    </div>
                </form>
