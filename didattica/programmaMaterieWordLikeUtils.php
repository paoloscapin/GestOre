...
function programmaMateriaWordLikeLooksLikeHtml(string $text): bool
{
    return preg_match('/<\/?(p|div|br|ul|ol|li|h[1-6]|strong|b|em|i|u|blockquote|span)\b/i', $text) === 1;
}

function programmaMateriaWordLikeIsUppercase(string $text): bool
{
    $text = trim($text);
    return preg_match('/\p{L}/u', $text) === 1 && preg_match('/\p{Ll}/u', $text) !== 1;
}

function programmaMateriaWordLikeSanitizeHtml(string $html): string
{
...
function programmaMateriaWordLikeLegacyTextToHtml(string $text): string
{
    $lines = preg_split('/\r\n|\r|\n/u', str_replace("\t", '  ', $text));
    if ($lines === false) {
        return '';
    ...
    foreach ($lines as $line) {
        ...
        foreach ($segments as $segment) {
            ...
            if (programmaMateriaWordLikeIsUppercase($raw)) {
    ...
    return programmaMateriaWordLikeSanitizeHtml($html);
}

function programmaMateriaWordLikeEnsureHtml($value): string
{
    ...
    return programmaMateriaWordLikeLooksLikeHtml($value)
        ? programmaMateriaWordLikeSanitizeHtml($value)
        : programmaMateriaWordLikeLegacyTextToHtml($value);
}
