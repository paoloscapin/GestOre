...
if ($ruoloEffettivo === 'genitore') {
  ...
  if (!programmiPubbliciGenitoreCanAccessStudent(intval($__genitore_id ?? 0), $publicStudentId)
    || !programmiPubbliciCanAccessProgram('materie', $programId, $publicStudentId)) {
    http_response_code(403);
  ...
  if (!programmiPubbliciCanAccessProgram('materie', $programId, $publicStudentId)) {
    http_response_code(403);
    exit('Programma non disponibile per questo studente.');
  }
}
...
$query = "SELECT * from programma_moduli WHERE id_programma = $programId ORDER BY ordine ASC";
