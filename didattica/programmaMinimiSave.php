...
function canEditProgrammaMinimiRecord(int $programmaId, int $materiaId = 0): bool
{
	...
	if ($is_docente_effettivo) {
		...
		if ($materiaId <= 0 && $programmaId > 0) {
			$program = dbGetFirst("SELECT id_materia FROM programma_minimi WHERE id=" . intval($programmaId));
...
if (isset($_POST)) {
	...
	if (!canEditProgrammaMinimiRecord($id, $materia_id)) {
	...
	if ($id > 0) {
		$query = "UPDATE programma_minimi SET anno = '$anno_id', id_indirizzo = '$indirizzo_id', id_materia = '$materia_id', id_utente = '$utente_id', updated = '$update' WHERE id = '$id'";
		dbExec($query);
		info("aggiornati obiettivi minimi materia id=$id  anno=$anno_id id_indirizzo=$indirizzo_id id_materia=$materia_id id_utente=$utente_id updated=$update");
	} else {
		$query = "INSERT INTO programma_minimi(anno, id_indirizzo, id_materia, id_utente, updated) VALUES('$anno_id', '$indirizzo_id', '$materia_id', '$utente_id', '$update')";
		dbExec($query);
		$id = dblastId();
		info("aggiunti obiettivi minimi materia id=$id  anno=$anno_id id_indirizzo=$indirizzo_id id_materia=$materia_id id_utente=$utente_id updated=$update");
	}
}
?>
